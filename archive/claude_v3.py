        ...
        while not self.stop_event.is_set():
            if self.big_running:
                try:
                    ...
                    # If we have a position, just click without moving mouse
                    if big_cookie_position:
                        # Check if mouse has moved from the big cookie position
                        current_position = pyautogui.position()
                        position_tolerance = 10  # Allow small movement within 10 pixels
                        ...
                        if position_difference[0] > position_tolerance or position_difference[1] > position_tolerance:
                            timestamp = datetime.now().strftime('%H:%M:%S')
                            print(f"[{timestamp}] Mouse moved by user, stopping big cookie clicker")
                            self.toggle_big_bot()  # Turn off the big cookie clicker
                            continue
                        
                        # Click directly without moving mouse (it's already on the cookie)
                        pyautogui.click()
            ...
            # Calculate sleep interval in seconds for clicking speed
            if big_cookie_position and self.big_running:
                # Use minimal delay between clicks for maximum speed
                time.sleep(0.001)  # Minimal sleep to prevent CPU hogging
            else:
                # Use normal interval when searching for cookie
